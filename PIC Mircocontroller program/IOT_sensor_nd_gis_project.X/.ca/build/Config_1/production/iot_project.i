...
void WIFI_Module_Init (void) {
    ...
    const unsigned char initCommand[] = "WIFI_MODULE_CMD_INIT";


    for (int i = 0; i < sizeof(initCommand) - 1; i++) {
        UART_Write(initCommand[i]);
    }
}


void WiFi_Module_Transmit(unsigned char *data)
{

     unsigned char sendCommand[] = "WIFI MODULE CMD SEND";


    for (int i = 0; i < (sendCommand) - 1; i++) {
        UART_Write(sendCommand[i]);
    }
