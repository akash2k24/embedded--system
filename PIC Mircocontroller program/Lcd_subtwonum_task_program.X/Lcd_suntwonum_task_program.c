...
unsigned char s,j,k[10],Equal,plus,minus;
...
void main()
{
    ...
    Lcd_Command(0x83);
    Lcd_Data(minus);  // To display '-' symbol in display
    Lcd_Command(0x84);
    ...
    {
    Lcd_Command(0x88);
    sum = num1-num2; //  To sum the num1 and num2 will be sub
    Lcd_Data(plus);  // if the numer is less than conition means to display '+'
    Lcdoutput(sum);  //To diaplay the output value
    }
    else
    {
        sum= num2 - num1; //  To sum the num1 and num2 will be sub
        Lcd_Data(minus); // if the numer is greater than conition means to display '-'
        Lcdoutput(sum);  //To diaplay the output value
    }
